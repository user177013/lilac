...
class LilacEnvironment(BaseModel):
  ...
  OSMANTHUS_DATA_PATH: str = PydanticField(
    description='[Deprecated] The Lilac data path where datasets, concepts, caches are stored. '
    'This is deprecated in favor of `OSMANTHUS_PROJECT_DIR`, but will work for backwards compat.'
  )
  OSMANTHUS_PROJECT_DIR: str = PydanticField(
    description='The Lilac project directory where datasets, concepts, caches are stored.'
    'This replaces `OSMANTHUS_PROJECT_DIR`, which is deprecated but as the same functionality. '
  ...
  LILAC_LOAD_ON_START_SERVER: str = PydanticField(
    description='When true, will load from osmanthus.yml upon startup.'
  )
...
def get_project_dir() -> str:
  ...
  project_dir = env('OSMANTHUS_PROJECT_DIR', None)
  if not project_dir:
    project_dir = env('OSMANTHUS_DATA_PATH', DEFAULT_PROJECT_DIR)
  if not project_dir:
    raise ValueError('`OSMANTHUS_PROJECT_DIR` environment variable must be set. ')
...
def set_project_dir(project_dir: Union[str, pathlib.Path]) -> None:
  ...
  os.environ['OSMANTHUS_PROJECT_DIR'] = str(project_dir)
